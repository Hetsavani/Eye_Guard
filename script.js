...
function start(temp) {
  if (!temp) {
    ...
    // Optionally, you can hide the video element

    // Alternatively, you can disable the autoplay attribute
    vid.removeAttribute("autoplay");
    vid.setAttribute("disabled", "disabled");
    ...
    // document.getElementById("status").innerHTML = "Sleep"
    Promise.all([
  ...
  setInterval(async () => {
    ...
    if (resizedDetections.length > 0) {
      ...
      if (areEyesClosed(landmarks)) {
        ...
        status.innerHTML = "Sleep"
        status.style.color = "red";
      } else {
        video.style.border = "0px solid red";
        status.innerHTML = "Awake"
